# hive-state
### Build decentralized apps on the Hive blockchain with ease!
...
The Hive blockchain is one of the fastest and most performant blockchains in existence and also has over 60,000 active users each day, with free and fast transactions. `hive-state`, makes it incredibly easy to create decentralized apps on the Steem blockchain using soft consensus.
...
`hive-state` uses the `custom_json` operation type to create soft-consensus transactions. You can read more at the [dev portal](https://developers.hive.io) by searching up `custom_json`.

#### hive-state
`hive-state` is a framework for building fully decentralized DApps using soft-consensus with the Steem blockchain. Using `hive-state` you can define events that occur when a new transaction of a certain type is created, such as updating the state, displaying feedback for the user, etc. Using these events, you can build a fully decentralized DApp. Look below for an example tutorial.
...
var steemState = require('hive-state'); // This will allow us to build our DApp.
var steemTransact = require('steem-transact'); // This makes it easy to make transactions on the Steem/Hive blockchain.
...
```
Then we will create some variables for our username and private key on the Hive blockchain (instead of public keys like Bitcoin and Ethereum, Hive uses usernames). We'll hardcode this for the sake of simplicity, but you could easily ask the user to input theirs. To complete this step you will have to have a Hive account (which can be done at https://hive.blog).
```
...
```
Now we need to actually create our interface with the Hive blockchain. Like others like `web3` (for Ethereum), Steem uses the `dsteem` package to interface with the Steem blockchain. We'll provide this with a node to connect to (similar to how Infura works on Ethereum) and to get the blockchain data from:

```
var client = new hive.Client();
```
...
```
Then actually create the hive-state instance. This method uses the arguments:

`client`: the dsteem client to get transactions from,

`hive`: a dhive instance,
...
`prefix`: the name of your DApp. This prefix is at the beginning of every transaction created for your DApp and ensures that no other DApps will use the same transaction ids as yours. Make sure to make this unique for your DApp! For example, [Splinterlands](https://splinterlands.com/), a highly successful Steem DApp, has the prefix `sm_`. We will use the prefix `basic_messaging_app_` for our app.
...
```
  var processor = hiveState(client, steem, result.head_block_number, 100, 'basic_messaging_app_', 'latest');
```
...
```
processor.on('message', function(json, from, active, promise_chain) {
  console.log(from, 'says:', json.message);
  promise_chain[0](promise_chain[2])
});
processor.start();
...
```
var transactor = steemTransact(client, hive, 'basic_messaging_app_');
```
...
```
var steem = require('dhive');
var steemState = require('hive-state');
